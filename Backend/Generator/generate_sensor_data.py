...
def generate_sensor_data(num_records):
    ...
    for _ in range(num_records):
        ...
        wind_speed = random.uniform(0.0, 10.0)
        incline_angle = random.uniform(-10.0, 10.0)
        latitude = random.uniform(locations[antenna]["latitude"] - 0.1, locations[antenna]["latitude"] + 0.1)
        longitude = random.uniform(locations[antenna]["longitude"] - 0.1, locations[antenna]["longitude"] + 0.1)
    ...
    print(f"Total de {len(data)} dados gerados.")

    for data_point in data:
        print(data_point)

    print(f"Total de {len(data)} dados exibidos.")
